...
class Monkey {
  public:
    ...
    virtual Monkey* simplify(unordered_map<string, Monkey*> *exprs) { return this;}
};

class Simple : public Monkey {
  public:
    double value;
    Simple(double i, string name) : Monkey(name) {
      this->value = i;
    }
...
class Expression : public Monkey {
  public:
    ...
    virtual double evaluate(unordered_map<string, Monkey*> exprs) { 
      ...
      auto r = exprs.find(right)->second;
      double result = 0;
      if (op == "+") result = l->evaluate(exprs) + r->evaluate(exprs);
      if (op == "-") result = l->evaluate(exprs) - r->evaluate(exprs);
      if (op == "*") result = l->evaluate(exprs) * r->evaluate(exprs);
      if (op == "/") result = l->evaluate(exprs) / r->evaluate(exprs);
      if (op == "=") {
...
int main() {
  ...
  long i = 3699945358464;
    ...
    if (i % 1 == 0) cout << "progress" << i << " " << result << "\n";
  }
  cout << i;
