...
class Directory {
  public:
    ...
    long size() {
      long s = 0;
      for (unordered_map<string, Directory>::iterator it = dirs.begin(); it != dirs.end(); ++it) {
        s += it->second.size();
      }
...
Directory* addDir(Directory *current, string name) {
        if (auto search = (*current).dirs.find(name); search != (*current).dirs.end()) {
...
long sumDirs(Directory *d, long input) {
  ...
  for (unordered_map<string, Directory>::iterator it = d->dirs.begin(); it != d->dirs.end(); ++it) {
...
//      cout << a << " " << b << " " << c << "\n";
      ...
      addDir(current, b);
        ...
        if (auto search = current->files.find(b); search == current->files.end()) {
          File *f = new File();
          f->length = stol(a);
          f->name = b;
